...
const callBuiltinByName = require('./module_builtin_extra5.js');
...
function evaluate(tree, genv, lenv) {
  ...
  if (tree[0] === 'var_decl') {
    ...
    const name = tree[1];
    if (name in lenv) {
      println('---ERROR: varibable ALREADY exist --:' + name);
      //printObj(lenv);
      abort();
    }
...
let genv = {
  ...
  'getLength' :  ['builtin', 'getLength'],
  'getTypeOf' :  ['builtin', 'getTypeOf'],
  'getKeys' :  ['builtin', 'getKeys'],
  'printWarn' :  ['builtin', 'printWarn'],
};
let lenv = {};
